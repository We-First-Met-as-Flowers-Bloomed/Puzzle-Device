...
- [x] 解压 STM32 工程，排除嵌套 `.git` 和编译输出，并复制到 `Firmware/STM32F407/`。
- [x] 解压 MaixCAM2 工程，删除缓存、临时目录、历史发布包和 `config.json`，保留 `config.example.json`。
- [x] 复制 EDA 工程、STL 文件和系统设计资料到对应目录。
- [x] 检查目录中不存在 `.git`、`__pycache__`、`.pytest_cache`、构建缓存和超过 100 MB 的文件。
...
- [x] 编写项目概述、功能、系统流程、目录树和技术组成。
- [x] 编写 STM32 烧录、MaixCAM2 配置部署、接线检查、上电初始化和一键启动步骤。
- [x] 在 `Demo/README.md` 和根 README 中加入 Bilibili 演示入口。
- [x] 标注当前未提供的 PCB Gerber、BOM 和 STEP 文件，避免读者误判。
...
- [x] 扫描敏感信息、绝对路径、缓存、嵌套仓库和超大文件。
- [x] 运行视觉模块测试；若依赖环境不满足，记录可复现的失败原因。
- [x] 检查 README 中的相对链接和目录引用均存在。
- [x] 更新开发日志，提交全部文件并推送 `main`。
- [x] 通过 GitHub API核对远程提交、README、许可证及主要目录。
